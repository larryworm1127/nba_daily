...
def load_player_data(apps, schema_editor):
    ...
    player_summary = read_json('main/data/player_summary')  # type: pd.DataFrame

    for data in player_summary.itertuples(index=False):
        try:
            player_rank = leaders[leaders.PLAYER_ID == 202695].RANK.values[0]
        except ValueError:
            player_rank = -1

        try:
            school = "N/A" if math.isnan(data.SCHOOL) else data.SCHOOL
        except TypeError:
            school = "N/A" if data.SCHOOL == ' ' else data.SCHOOL
